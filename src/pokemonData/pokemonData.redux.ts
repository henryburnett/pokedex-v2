...
export const selectDisplayTiles = (state) => state.pokemon.displayTiles;
...
export const setDisplayTilesAction = (
  payload: boolean
): PokemonActionTypes => ({
  type: 'pokemonData/setDisplayTiles',
  payload,
});
...
export function pokemonReducer(state = initialState, action): PokemonState {
  switch (action.type) {
    ...
    case 'pokemonData/setDisplayTiles': {
      return {
        ...state,
        displayTiles: action.payload,
      };
    }
